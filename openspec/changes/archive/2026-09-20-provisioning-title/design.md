...
## По итогам ревью Codex (PR #15)

- **P1, снимок без `title` во время раската.** Старая реплика продюсера
  шлёт более новую версию без поля, и консьюмер, заменяющий снимок
  целиком, стирал должность. Правило для необязательных полей
  зафиксировано в контракте: отсутствие поля — «не передано», значение
  сохраняется; явная пустая строка — очищает. Референсный консьюмер
  запоминает, было ли поле в теле; SQL-пример показывает
  `COALESCE($4, identity_users.title)`.
- **P2, SQL-пример без `title`.** Дополнен вместе с правилом выше.
