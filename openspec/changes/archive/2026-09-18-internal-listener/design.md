...
6. **Стенд.** `values-local.yaml`: `public.extraFrom` — поды с
   ...
   остаётся: он про HTTPRoute. Probe-поды — долгоживущие (`sleep` +
   `kubectl exec`), не `kubectl run -i`: выяснилось на стенде, что CNI
   добавляет адрес нового пода в набор peers политики асинхронно, и
   одноразовый под с верными метками получал отказ; после Ready адрес
   учтён (проверено: 401/200 с +0 с). Селектор по namespace этой гонке не
   подвержен — прокси долгоживущие.
...
## По итогам ревью Codex (PR #11)

- **P2, shutdown обеих зон — параллельно.** Первая реализация вызывала
  `Shutdown` последовательно: пока публичный слушатель дренировал долгий
  запрос, внутренний продолжал принимать ext-auth и получал остаток общего
  таймаута (или уже истёкший контекст). Теперь оба `Shutdown` стартуют
  одновременно с одним дедлайном; ошибки собираются через `errors.Join`.
  Регрессионный тест: при медленном публичном запросе внутренний слушатель
  перестаёт принимать соединения сразу после сигнала остановки.
