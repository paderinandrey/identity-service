...
- [ ] 3.1 `stand-compose-supergraph.sh`: curl-под с меткой tools, e2e-сессия с `:8081`, SDL с `:8080`; `mise run stand:up` проходит целиком
- [ ] 3.2 `stand-verify.sh`: шаги «validate на публичном порту → 404», «validate на внутреннем порту: из пода без метки соединение не устанавливается, из пода с меткой → 401», «подделанные `x-identity-user-id`/`x-identity-permissions` с cookie → echo показывает id сессии и пустые права»; нумерация шагов и итоговая строка обновлены
- [ ] 3.3 `mise run stand:verify` зелёный целиком; если шаг с пустыми правами падает — остановиться и зафиксировать в design (риск 1), не ослаблять проверку
