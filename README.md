...
`docs/schema/` is generated by [tbls](https://github.com/k1LoW/tbls)
from a migrated database: one page per table with columns, the
`COMMENT ON` text, constraints and indexes, plus a Mermaid ER diagram in
`docs/schema/README.md`. It is the Rails `annotate` equivalent for "open
it and see the schema". Regenerate after adding a migration:

```bash
mise run schema:docs    # migrate the local database, rewrite docs/schema
mise run schema:check   # tbls diff — CI runs this and fails on stale docs
```
